...
void serializeSophusSE3(Archive &ar, Sophus::SE3f &T, const unsigned int version)
{
    ...
    ar & slamcpp::serialization::make_array(quat.data(), quat.size());
    ar & slamcpp::serialization::make_array(transl.data(), transl.size());
...
void serializeDiagonalMatrix(Archive &ar, Eigen::DiagonalMatrix<float, dim> &D, const unsigned int version)
{
    ...
    ar & slamcpp::serialization::make_array(dense.data(), dense.size());
...
void serializeMatrix(Archive& ar, cv::Mat& mat, const unsigned int version)
{
    ...
    if (continuous) {
        ...
        ar & slamcpp::serialization::make_array(mat.ptr(), data_size);
    } else {
        const unsigned int row_size = cols*mat.elemSize();
        for (int i = 0; i < rows; i++) {
            ar & slamcpp::serialization::make_array(mat.ptr(i), row_size);
        }
    }
}
