...
#include "slamcpp/SerializationUtils.h"
...
class Bias
{
    friend class slamcpp::serialization::access;
...
class Calib
{
    ...
    void serialize(Archive & ar, const unsigned int version)
    {
        ...
        ar & slamcpp::serialization::make_array(Cov.diagonal().data(), Cov.diagonal().size());
        ar & slamcpp::serialization::make_array(CovWalk.diagonal().data(), CovWalk.diagonal().size());
...
class Preintegrated
{
    ...
    {
        ar & dT;
        ar & slamcpp::serialization::make_array(C.data(), C.size());
        ar & slamcpp::serialization::make_array(Info.data(), Info.size());
        ar & slamcpp::serialization::make_array(Nga.diagonal().data(), Nga.diagonal().size());
        ar & slamcpp::serialization::make_array(NgaWalk.diagonal().data(), NgaWalk.diagonal().size());
        ar & b;
        ar & slamcpp::serialization::make_array(dR.data(), dR.size());
        ar & slamcpp::serialization::make_array(dV.data(), dV.size());
        ar & slamcpp::serialization::make_array(dP.data(), dP.size());
        ar & slamcpp::serialization::make_array(JRg.data(), JRg.size());
        ar & slamcpp::serialization::make_array(JVg.data(), JVg.size());
        ar & slamcpp::serialization::make_array(JVa.data(), JVa.size());
        ar & slamcpp::serialization::make_array(JPg.data(), JPg.size());
        ar & slamcpp::serialization::make_array(JPa.data(), JPa.size());
        ar & slamcpp::serialization::make_array(avgA.data(), avgA.size());
        ar & slamcpp::serialization::make_array(avgW.data(), avgW.size());

        ar & bu;
        ar & slamcpp::serialization::make_array(db.data(), db.size());
        ar & mvMeasurements;
    }
...
private:
    ...
    struct integrable
    {
        ...
        void serialize(Archive & ar, const unsigned int version)
        {
            ar & slamcpp::serialization::make_array(a.data(), a.size());
            ar & slamcpp::serialization::make_array(w.data(), w.size());
            ar & t;
        }
