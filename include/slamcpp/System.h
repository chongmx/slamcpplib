...
#include "slamcpp/LocalMapping.h"
...
public:
    ...
    // Read-only handle on the atlas, so a host application can draw the map
    // itself. This fork builds no Pangolin Viewer/MapDrawer, and the tracked
    // point accessors above only ever describe the current frame, so without
    // this there is no way to reach the key frames, the full point cloud or
    // the covisibility graph. Every Atlas/Map/KeyFrame/MapPoint getter takes
    // its own lock, so readers do not need external synchronisation.
...
private:
    ...
    // Archive formats for the atlas, as FileType values, from the settings
    // file. System.AtlasFormat sets both; System.AtlasSaveFormat overrides the
    ...
    // The Pangolin-based Viewer and MapDrawer are not built in this fork; the
    // host application does its own visualisation. These members are kept as
    // pointers to incomplete types (always NULL) so the ABI and the
