...
protected:
    ...
    // A generation counter, not a flag, despite the name: RunGlobalBundleAdjustment
    // snapshots it, and bails out if it has moved by the time the adjustment
    // finishes. Declared bool upstream, where ++ saturates at true, so only the
    // first abort was ever observed and every later one was silently missed.
    // Widening it to int both restores that intent and compiles under C++17,
    // which removed ++ on bool outright.
    int mnFullBAIdx;
