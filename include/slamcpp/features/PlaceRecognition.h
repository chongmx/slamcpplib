...
public:
    ...
    // Descriptor-free entry points, so that KeyFrame never has to hold a
    // torch::Tensor and therefore never has to include libtorch. The upstream
    // fork kept the global descriptor on the key frame itself, which drags
    // torch headers into every translation unit that touches a key frame and
    // makes the class's layout depend on whether the build has libtorch.
    // Holding them here instead costs a lookup and keeps the core clean.
    void addFrom(KeyFrame *pKF, const cv::Mat &image);

    bool has(KeyFrame *pKF);

    std::vector<KeyFrame*> queryStored(KeyFrame *pKF, int nCandidates,
                                       const std::set<KeyFrame*> &spConnectedKFs);
