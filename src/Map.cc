...
void Map::PreSave(std::set<GeometricCamera*> &spCams)
{
    ...
    // Only the key frames that are actually written below. Recording the id of
    // a bad one leaves a reference in the file that PostLoad cannot resolve,
    // and an unresolvable origin becomes a null pointer that later gets
    // dereferenced. See KeyFrame::PreSave for the same trap.
    auto willBeSaved = [this](KeyFrame* pKF) {
        return pKF && !pKF->isBad() && mspKeyFrames.find(pKF) != mspKeyFrames.end();
    };
    ...
    {
        if(willBeSaved(mvpKeyFrameOrigins[i]))
    ...
    if(willBeSaved(mpKFinitial))
    {
        mnBackupKFinitialID = mpKFinitial->mnId;
    }

    mnBackupKFlowerID = -1;
    if(willBeSaved(mpKFlowerID))
    {
...
void Map::PostLoad(KeyFrameDatabase* pKFDB, ORBVocabulary* pORBVoc/*, map<long unsigned int, KeyFrame*>& mpKeyFrameId*/, map<unsigned int, GeometricCamera*> &mpCams)
{
    ...
    // find() throughout: operator[] would insert a null for a missing id and
    // hand it back as if it were a real key frame.
    auto resolve = [&mpKeyFrameId](long unsigned int id) -> KeyFrame* {
        map<long unsigned int, KeyFrame*>::iterator it = mpKeyFrameId.find(id);
        return it == mpKeyFrameId.end() ? static_cast<KeyFrame*>(NULL) : it->second;
    };

    if(mnBackupKFinitialID != -1)
    {
        mpKFinitial = resolve(mnBackupKFinitialID);
    }

    if(mnBackupKFlowerID != -1)
    {
        mpKFlowerID = resolve(mnBackupKFlowerID);
    }
    ...
    {
        if(KeyFrame* pKFi = resolve(mvBackupKeyFrameOriginsId[i]))
            mvpKeyFrameOrigins.push_back(pKFi);
    }
