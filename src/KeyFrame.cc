...
{
    // Membership in spKF is not the same question as "will this be saved".
    // Map::PreSave writes only the key frames that are not bad, while spKF is
    // the map's whole set, bad ones included. Recording an id that no saved
    // object backs leaves a dangling reference in the file, and PostLoad then
    // turns it into a null pointer that UpdateBestCovisibles dereferences.
    auto willBeSaved = [&spKF](KeyFrame* pKF) {
        return pKF && !pKF->isBad() && spKF.find(pKF) != spKF.end();
    };
    auto pointWillBeSaved = [&spMP](MapPoint* pMP) {
        return pMP && !pMP->isBad() && spMP.find(pMP) != spMP.end();
    };
    ...
    for(int i = 0; i < N; ++i)
    {
        ...
        if(pointWillBeSaved(mvpMapPoints[i]))
    ...
    //Inertial data
    mBackupPrevKFId = -1;
    if(willBeSaved(mPrevKF))
        mBackupPrevKFId = mPrevKF->mnId;

    mBackupNextKFId = -1;
    if(willBeSaved(mNextKF))
...
void KeyFrame::PostLoad(map<long unsigned int, KeyFrame*>& mpKFid, map<long unsigned int, MapPoint*>& mpMPid, map<unsigned int, GeometricCamera*>& mpCamId){
    ...
    // Looked up rather than indexed. operator[] on a std::map inserts a null
    // pointer for an id that is not there and hands it back, which is how a
    // file with one dangling reference used to become a null key frame in the
    // covisibility graph and a segfault in UpdateBestCovisibles. A reference
    // that cannot be resolved is dropped instead.
    auto resolve = [&mpKFid](long unsigned int id) -> KeyFrame* {
        map<long unsigned int, KeyFrame*>::iterator it = mpKFid.find(id);
        return it == mpKFid.end() ? static_cast<KeyFrame*>(NULL) : it->second;
    };
    ...
    for(map<long unsigned int, int>::const_iterator it = mBackupConnectedKeyFrameIdWeights.begin(), end = mBackupConnectedKeyFrameIdWeights.end();
        it != end; ++it)
    {
        KeyFrame* pKFi = resolve(it->first);
        if(pKFi)
    ...
    {
        mPrevKF = resolve(mBackupPrevKFId);
    }
    if(mBackupNextKFId != -1)
    {
        mNextKF = resolve(mBackupNextKFId);
    }
