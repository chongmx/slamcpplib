...
// FileType is the caller-facing spelling; Format is the archive's. Kept
// separate so System's public enum does not drag the serializer's header into
// everything that includes System.h.
slamcpp::serialization::Format ArchiveFormat(int type)
{
    switch(type)
    {
        case ORB_SLAM3::System::TEXT_FILE: return slamcpp::serialization::Format::Text;
        case ORB_SLAM3::System::XML_FILE:  return slamcpp::serialization::Format::Xml;
        default:                           return slamcpp::serialization::Format::Binary;
    }
}

// Accepts the spellings used in the settings file.
int ParseArchiveFormat(const std::string& name, int fallback)
{
    if(name == "binary") return ORB_SLAM3::System::BINARY_FILE;
    if(name == "text")   return ORB_SLAM3::System::TEXT_FILE;
    if(name == "xml")    return ORB_SLAM3::System::XML_FILE;
    std::cerr << "Unknown atlas format '" << name << "', keeping the previous one" << std::endl;
    return fallback;
}

const char* ArchiveFormatName(int type)
{
    switch(type)
    {
        case ORB_SLAM3::System::TEXT_FILE: return "text";
        case ORB_SLAM3::System::XML_FILE:  return "xml";
...
System::System(const string &strVocFile, const string &strSettingsFile, const eSensor sensor,
    ...
    mSensor(sensor), mpViewer(static_cast<Viewer*>(NULL)), mpMapDrawer(static_cast<MapDrawer*>(NULL)),
    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    // Read straight from the file rather than through Settings, so that it
    // works for both the 1.0 settings path and the legacy one above.
    node = fsSettings["System.AtlasFormat"];
    if(!node.empty() && node.isString())
    {
        mAtlasFileType = ParseArchiveFormat((string)node, BINARY_FILE);
    }

    // Defaults to whatever the atlas is read as, so setting one key is enough
    // for the common case of reading and writing the same format.
    mAtlasSaveFileType = mAtlasFileType;
    node = fsSettings["System.AtlasSaveFormat"];
    if(!node.empty() && node.isString())
    {
        mAtlasSaveFileType = ParseArchiveFormat((string)node, mAtlasFileType);
    }
    ...
    if(mStrLoadAtlasFromFile.empty())
    {
        ...
        // Raises rather than exit(-1)ing: this is a library, and the caller
        // is in a better position than we are to decide what a bad vocabulary
        // path means for the process.
        if(!LoadVocabulary(*mpVocabulary, strVocFile))
        {
            throw std::runtime_error("could not load the vocabulary at: " + strVocFile);
        }
    ...
    else
    {
        ...
        bool isRead = LoadAtlas(mAtlasFileType);
    ...
    //Create Drawers. FrameDrawer is used by the host application; the
    //Pangolin-based MapDrawer/Viewer have been removed from this build.
    ...
    //The Pangolin viewer has been removed from this build; bUseViewer is
    //accepted for API compatibility but ignored. The host application is
...
void System::SaveAtlas(int type){
    if(!mStrSaveAtlasToFile.empty())
    {
        ...
        const slamcpp::serialization::Format format = ArchiveFormat(type);

        cout << "Starting to write the save file (" << ArchiveFormatName(type) << ")" << endl;
        std::remove(pathSaveFileName.c_str());
        std::ofstream ofs(pathSaveFileName, std::ios::binary);
        {
            slamcpp::serialization::OutputArchive oa(ofs, format);
            oa << strVocabularyName;
            oa << strVocabularyChecksum;
            oa << mpAtlas;
        }
        cout << "End to write the save file" << endl;
    }
}
...
bool System::LoadAtlas(int type)
{
    ...
    cout << "Starting to read the save file (" << ArchiveFormatName(type) << ")" << endl;
    ...
    slamcpp::serialization::InputArchive ia(ifs, ArchiveFormat(type));
    ia >> strFileVoc;
    ia >> strVocChecksum;
    ia >> mpAtlas;
    cout << "End to read the save file" << endl;
    isRead = true;
