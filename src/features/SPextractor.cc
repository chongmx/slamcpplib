...
// ExtractorNode::DivideNode is defined once, in ORBextractor.cc.


SPextractor::SPextractor(int _nfeatures, float _scaleFactor, int _nlevels,
         float _iniThFAST, float _minThFAST, const std::string& modelPath):
    iniThFAST(_iniThFAST), minThFAST(_minThFAST), mModelPath(modelPath)
{
    // Assigned rather than listed: these belong to FeatureExtractor now.
    nfeatures = _nfeatures;
    scaleFactor = _scaleFactor;
    nlevels = _nlevels;

    // The weights used to be opened as a bare relative path, so the library
    // only worked when the process happened to be started next to the file.
    if(mModelPath.empty())
        mModelPath = std::string(SLAMCPP_RESOURCE_DIR) + "/models/superpoint.pt";

    {
        std::ifstream probe(mModelPath, std::ios::binary);
        if(!probe.good())
            throw std::runtime_error("slamcpp: SuperPoint weights not found at " + mModelPath);
    }
    ...
    torch::Device load_device = torch::cuda::is_available() ? torch::kCUDA : torch::kCPU;
    {
        ...
        torch::load(model, mModelPath, load_device);
...
{
    // SuperPoint has no fisheye stereo overlap handling, so the band is not
    // used and every key point counts as outside it.
    (void)vLappingArea;
