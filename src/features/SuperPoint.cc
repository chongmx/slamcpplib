...
void SPDetector::computeDescriptors(const std::vector<cv::KeyPoint> &keypoints, cv::Mat &descriptors)
{
    ...
    // Unit-normalise each descriptor.
    //
    // desc is [256, n_keypoints] here, so the descriptor runs down dim 0 and
    // the key point index across dim 1. Taking the norm along dim 1, as this
    // did, divides each of the 256 channels by its spread across the whole key
    // point set, which is not a per-descriptor normalisation at all: it leaves
    // descriptors whose length depends on how many key points the frame
    // happened to yield. The matcher's L2 thresholds assume unit vectors, so
    // matching degraded frame to frame and tracking did not survive.
    auto dn = torch::norm(desc, 2, 0);
    desc = desc.div(torch::unsqueeze(dn, 0).clamp_min(1e-12));
