...
void PlaceRecognition::addFrom(KeyFrame *pKF, const cv::Mat &image)
{
    if(!mbLoaded || image.empty())
        return;

    add(pKF, extractDescriptor(image));
}

bool PlaceRecognition::has(KeyFrame *pKF)
{
    std::lock_guard<std::mutex> lock(mMutex);
    for(const Entry &e : mvDatabase)
        if(e.pKF == pKF)
            return true;
    return false;
}

std::vector<KeyFrame*> PlaceRecognition::queryStored(KeyFrame *pKF, int nCandidates,
                                                     const std::set<KeyFrame*> &spConnectedKFs)
{
    ...
    if(!descriptor.defined() || descriptor.numel() == 0)
        return std::vector<KeyFrame*>();

    return query(pKF, descriptor, nCandidates, spConnectedKFs);
}
