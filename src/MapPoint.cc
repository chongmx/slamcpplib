...
{
    // See KeyFrame::PreSave: being in the set is not the same as being saved,
    // because Map::PreSave writes only the entries that are not bad.
    auto willBeSaved = [&spKF](KeyFrame* pKF) {
        return pKF && !pKF->isBad() && spKF.find(pKF) != spKF.end();
    };

    mBackupReplacedId = -1;
    if(mpReplaced && !mpReplaced->isBad() && spMP.find(mpReplaced) != spMP.end())
...
{
    // find() rather than operator[], which would insert a null for a missing id.
    mpRefKF = static_cast<KeyFrame*>(NULL);
    {
        map<long unsigned int, KeyFrame*>::iterator it = mpKFid.find(mBackupRefKFId);
        if(it != mpKFid.end())
            mpRefKF = it->second;
    }
    if(!mpRefKF)
    ...
    {
        map<long unsigned int, KeyFrame*>::iterator itKF = mpKFid.find(it->first);
        KeyFrame* pKFi = (itKF == mpKFid.end()) ? static_cast<KeyFrame*>(NULL) : itKF->second;
